...
export default function BookingDetails({ route}: any) {
  ...
  const {data, error, loading, refetch} = useQuery(GET_BOOKING, {
    variables: { getBookingId: id },
  })
  const [createCheckoutSession, {loading: checkoutLoading, error: checkoutError}] = useMutation(
...
//   );
  ...
  const handleCall = () => {
    if (booking.property.realtor?.phone) {
      Linking.openURL(`tel:${booking.property.realtor.phone}`);
    }
  };

  const handleEmail = () => {
    if (booking.property.realtor?.email) {
      Linking.openURL(`mailto:${booking.property.realtor.email}`);
    }
  };
  ...
  const createPayment = async() => {
    try {
      if(!booking.id || openPayment) return
      ...
      })}/--/payment-cancel?bookingId=${booking.id}`;
       ...
       onScroll={handleScroll} contentContainerStyle={{paddingBottom: 100}} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.locationRow}>
            ...
            <Text style={[styles.locationText, { color: theme.colors.textSecondary }]}>
              {booking.property?.address.street}, {booking.property?.address.city}
            </Text>
          </View>
        ...
        <View style={[styles.card, { backgroundColor: theme.colors.card }]}>
          ...
          >
            <Text style={[styles.referenceText, { color: theme.colors.text }]}>
              {booking?.bookingReference || 'SOIWNW'}
            </Text>
            <Ionicons
              name="copy-outline"
              size={20}
          ...
          {showQR && (
            <View style={styles.qrContainer}>
              <QRCode
                value={booking?.bookingReference || 'SOOWIW'}
                size={200}
        ...
        {true && (
          <View style={[styles.card, { backgroundColor: theme.colors.card }]}>
            ...
            {true && (
              <Text style={[styles.instructions, { color: theme.colors.textSecondary }]}>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Expedita temporibus accusamus nobis sed modi cupiditate.
              </Text>
            )}
          </View>
        )}
        ...
        {booking.property?.realtor && (
          <View style={[styles.card, { backgroundColor: theme.colors.card }]}>
            ...
            <View style={styles.hostInfo}>
              <Image
                source={{
                  uri: booking.property.realtor?.avatar || 'https://via.placeholder.com/50',
                }}
                style={styles.hostAvatar}
              />
              <View style={styles.hostDetails}>
                <Text style={[styles.hostName, { color: theme.colors.text }]}>
                  {booking.property.realtor.name}
                </Text>
                {booking.property.realtor?.responseTime && (
                  <Text style={[styles.hostMeta, { color: theme.colors.textSecondary }]}>
                    Usually responds in {booking.realtor.responseTime}
                  </Text>
                )}
              </View>
            </View>
            ...
            <View style={styles.contactButtons}>
              ...
              <TouchableOpacity
                style={[styles.contactButton, { backgroundColor: theme.colors.backgroundSec }]}
                onPress={() => router.push({
                  pathname: '/(guest)/chats',
                  params: {chat: JSON.stringify({
                    recipientId: booking.property.realtor.id,
        ...
        <View style={[styles.card, { backgroundColor: theme.colors.card }]}>
            ...
            <View key={payment.id} style={styles.paymentRow}>
              <View style={styles.paymentLeft}>
                <Text style={[styles.paymentType, { color: theme.colors.text }]}>
                  {payment.status.charAt(0).toUpperCase() + payment.status.slice(1)} Payment
                </Text>
                <Text style={[styles.paymentDate, { color: theme.colors.textSecondary }]}>
                  {new Date(parseInt(payment.created_at)).toString()}
                </Text>
              </View>
          ...
          <View style={[styles.totalRow, { borderTopColor: theme.colors.border }]}>
            ...
            <Text style={[styles.totalAmount, { color: theme.colors.primary }]}>
              {booking.currency} {booking.amountPaid.toLocaleString()}
            </Text>
          </View>
          ...
          {mockBookingsData[0]?.securityDeposit && (
            <View style={[styles.depositBox, { backgroundColor: theme.colors.backgroundSec }]}>
              <View style={styles.depositRow}>
                ...
                <Text style={[styles.depositAmount, { color: theme.colors.text }]}>
                  {mockBookingsData[0].currency} {mockBookingsData[0].securityDeposit.amount}
                </Text>
              </View>
              <Text style={[styles.depositStatus, { color: theme.colors.textSecondary }]}>
                Status: {mockBookingsData[0].securityDeposit.status}
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.actionsContainer}>
          {(booking.payment_status === 'partial' || booking.payment_status === 'unpaid') && (
            <TouchableOpacity
          ...
          {true && (
            <TouchableOpacity
              style={[styles.secondaryButton, { borderColor: theme.colors.border }]}
              onPress={() => router.push({
                pathname: '/(guest)/(modals)/review/[booking]',
                params: {booking: JSON.stringify({
                  bookingId: booking.id,
                  thumbnail: 'https://res.cloudinary.com/dajzo2zpq/image/upload/v1752247182/properties/wlf2uijbultztvqptnka.jpg',
                  checkIn: booking.checkIn,
                  checkOut: booking.checkOut,
                  city: booking.property.address.city,
