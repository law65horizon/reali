...
const Specialize = () => {
    ...
    const {setField, specialize} = useHomeStore()
...
const eventsIcons = [
  ...
  return (
    <ThemedView plain secondary style={{width, height, gap: 50, paddingVertical: 40, paddingHorizontal: 22,  }}> 
           ...
           renderItem={({item}) => (
            <View style={[styles.shadow, {width: width/2-30, height: 100, backgroundColor: theme.colors.background2, justifyContent: 'center',
              ...
              borderWidth: category === item.name ? 2.5:1
                ...
                <Pressable onPress={() => category === item.name ? setCategory(''): setCategory(item.name)} style={{width: '100%'}}>
                    {item.icon()}
                    <ThemedText type='defaultSemiBold'> {item.name} </ThemedText>
                </Pressable>
            </View>
           )}
           contentContainerStyle={{ gap: 15, paddingBottom: 20, justifyContent: 'space-between'}}
        ...
        />
        <PreviousNextUI style={ {position: 'absolute', width, bottom: 100, zIndex: 1, }} nextFunc={() => handle_navigation('next')} disabled={!category} />
    </ThemedView>
  )
}
