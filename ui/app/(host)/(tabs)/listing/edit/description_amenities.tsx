...
export default function CreatePropertyStep2() {
  ...
  const descriptionData = usePropertyStore((state) => state.description)
  const setField = usePropertyStore((state) => state.setField)
  
  const [description, setDescription] = useState(descriptionData?.description || '');
  const [selectedAmenities, setSelectedAmenities] = useState<string[]>(descriptionData?.amenities || []);
  ...
  const handleNext = () => {
    if (validateStep() || 2==2) {
      ...
      if (stepData !== descriptionData) setField('description', stepData)

      router.push({
        pathname: '/listing/edit/photos',
      });
    }
  };
