...
export default function UpcomingTabScreen() {
  ...
  const { data, loading, refetch } = useQuery(GET_UPCOMING_BOOKINGS, {
    variables: {
      input: {...filters, status: filters?.status??'confirmed'}
    }
  });
    ...
    >
      {(loading && bookings.length === 0) ? (
        ...
        <View style={styles.emptyState}>
          ...
          <Text style={[styles.emptyTitle, { color: theme.colors.text }]}>No upcoming bookings</Text>
