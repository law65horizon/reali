...
const ChatScreen = () => {
  ...
  // Fetch messages
  const { subscribeToMore, data: messagesData, loading, refetch } = useQuery(MESSAGES_QUERY, {
  ...
  const { data: convData } = useQuery(CONVERSATION_QUERY, {
    ...
    update: (cache, { data }) => {
      ...
      if (existingData?.messages) {
        ...
        cache.writeQuery({
          ...
          variables: { conversationId, limit: 50 },
          data: {
            messages: [...filteredMessages, data.sendMessage],
          },
        });
      }
    },
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToMore({
      ...
      updateQuery: (prev, { subscriptionData }) => {
        ...
        if (prev.messages.some((m: Message) => m.id === newMessage.id)) {
          return prev;
        }
