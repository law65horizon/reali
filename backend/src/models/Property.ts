...
export class PropertyModel {
  ...
  private async getOrCreateAddress(
    ...
    const countryResult = await client.query(
      `INSERT INTO countries (name, code)
       ...
       RETURNING id`,
      [address.country.toLowerCase(), address.country.slice(0, 2).toUpperCase()]
    );
    const countryId = countryResult.rows[0].id;

    console.log({countryId}, address.country)
    // Get or create city
    const cityResult = await client.query(
      `INSERT INTO cities (name, country_id)
       ...
       RETURNING id`,
      [address.city.toLowerCase(), countryId]
    );
    ...
    try {
      ...
      const propertyResult = await client.query(
        ...
        [
          ...
          // property.sale_status.toLowerCase() || "rent",
          "rent"
        ]
      );
  ...
  let searchScoreField = ""
  if (query) {
    params.push(`%${query}%`);
    conditions.push(`(
      a.street % $${params.length}
      OR c.name % $${params.length}
      OR co.name % $${params.length}
      OR a.postal_code ILIKE '%' || $${params.length} || '%'
    )`);

    searchScoreField = `,
      (
        similarity(p.title, $${params.length}) * 1.5 +
        similarity(a.street, $${params.length}) * 1.3 +
        similarity(c.name, $${params.length}) * 1.1 +
        similarity(co.name, $${params.length})
      ) AS search_score,
    `;
  }
  ...
  let orderByClause = query
    ? "ORDER BY search_score DESC, rt.created_at DESC"
    : "ORDER BY rt.created_at DESC, rt.id DESC"

  // Main query
  const roomTypesQuery = `
    SELECT 
      ${fields.join(", ")} ${searchScoreField}
      ${hasDateFilter 
  ...
  console.log({roomTypesQuery, params, requestedFields});
  ...
  async addRoomUnit(input: Omit<Unit, "id">): Promise<Unit> {
    const result = await this.pool.query(
      `INSERT INTO room_units (room_type_id, unit_code, status)
       VALUES ($1, $2, $3)
       RETURNING *`,
      [input.roomTypeId, input.unitCode || null, input.status || "inactive"]
    );
